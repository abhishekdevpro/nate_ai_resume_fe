...
import { setEmployerLoginValues } from "../../state/reducer/employerLoginSlice";
...
const EmployerLogin = () => {
  ...
  const EmployerLoginValues = useSelector(
    (state) => state.employerLogin.employerLoginValues
  );
  const handleChange = (e) => {
    const { name, value } = e.target;
    dispatch(setEmployerLoginValues({ ...EmployerLoginValues, [name]: value }));
  };
  const handleSubmit = async (e) => {
    const reqBody = {
      email: EmployerLoginValues.email,
      password: EmployerLoginValues.password,
    };
    e.preventDefault();
    await axios({
      method: "POST",
      url: "https://api.resumesquad.net/api/employee/auth/login",
      headers: {
      ...
      .then((response) => {
        ...
        localStorage.setItem("employeeAuthToken", response.data.data.token);
        navigate("/employee");
      })
      .catch((err) => {
        console.log(err);
            ...
            <div className="flex flex-col my-2 gap-2 w-full items-center">
              <div className="flex flex-col gap-2 w-[90%]">
                ...
                <input
                  ...
                  id="email"
                  autoComplete="false"
                  value={EmployerLoginValues.email}
              ...
              <div className="flex flex-col gap-2 w-[90%]">
                ...
                <div className="w-full bg-white flex items-center px-2 py-2 rounded-md">
                  <input
                    ...
                    id="password"
                    autoComplete="false"
                    value={EmployerLoginValues.password}
                    onChange={handleChange}
              ...
              <Link to="/employee/forgot-password" className="w-[90%]">
