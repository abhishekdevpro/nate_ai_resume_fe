// SkillComponent.js
import React, { useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import {
  setSkillsValues,
  setSkillsData,
  deleteSkill,
} from "../../../../state/reducer/createResumeSlice";
import {
  FaArrowDown,
  FaArrowTrendDown,
  FaArrowUp,
  FaCirclePlus,
  FaCircleXmark,
  FaTrash,
} from "react-icons/fa6";
import { FaEdit } from "react-icons/fa";
import { setSkillInput } from "../../../../state/reducer/skillsInputSlice";

function SkillComponent(props) {
  const skillsValues = useSelector(
    (state) => state.createResumeSlice.skillsValues
  );
  const skillsData = useSelector((state) => state.createResumeSlice.skillsData);
  const dispatch = useDispatch();
  const inputValues = useSelector((state) => state.skillInputs);

  const [addItem4, setAddItem4] = useState(false);
  const showAddItem4 = () => {
    setAddItem4((prev) => !prev);
  };

  const handleSkillChange = (event) => {
    dispatch(setSkillsValues({ skills: event.target.value }));
  };

  //Edit Item Function
  const [editIndex, setEditIndex] = useState(-1);

  const updateItem = () => {
    if (editIndex !== -1) {
      const updatedFormData = [...skillsData];
      updatedFormData[editIndex] = skillsValues.skills;
      dispatch(setSkillsData(updatedFormData));
      dispatch(setSkillInput(""));
      dispatch(setSkillsValues({ skills: "" }));
      setEditIndex(-1);
    }
  };

  const editItem = (index) => {
    dispatch(setSkillsValues({ skills: skillsData[index] }));
    dispatch(setSkillInput(skillsData[index]));
    setEditIndex(index);
  };

  //Editing Functionality Ends here
  const handleAddSkill = () => {
    if (editIndex !== -1) {
      return;
    }
    const trimmedSkill = skillsValues.skills.trim();
    if (trimmedSkill) {
      const newSkillData = [...skillsData, trimmedSkill];
      dispatch(setSkillsData(newSkillData));
      dispatch(setSkillsValues({ skills: "" }));
      dispatch(setSkillInput(""));
    }
  };

  // shifting functionality for changing the index of the data

  const moveItemUp = (index) => {
    if (index === 0) return;
    const newSkills = [...skillsData];
    [newSkills[index], newSkills[index - 1]] = [
      newSkills[index - 1],
      newSkills[index],
    ];
    dispatch(setSkillsData(newSkills));
  };

  const moveItemDown = (index) => {
    if (index === skillsData.length - 1) return;
    const newSkills = [...skillsData];
    [newSkills[index], newSkills[index + 1]] = [
      newSkills[index + 1],
      newSkills[index],
    ];
    dispatch(setSkillsData(newSkills));
  };
  return (
    <div className="w-full flex flex-col mb-5 items-start">
      <h2 className="text-[17px] font-semibold">Skills</h2>
      <p>
        Highlight specific skills that show you fit the position. Make Sure they
        match the key skills mentioned in the job listing (especially when
        applying via an online system)
      </p>
      <h3
        onClick={showAddItem4}
        className="font-semibold my-3 flex gap-2 cursor-pointer items-center"
      >
        Add Item {addItem4 ? <FaCircleXmark /> : <FaCirclePlus />}
      </h3>
      {addItem4 ? (
        <div className="flex gap-x-6 flex-wrap items-baseline w-full">
          <input
            className="w-4/5 p-[10px] bg-cosretBlue-300 px-3 text-black text-[12px] outline-none border-[0.5px] border-[#323232] rounded-lg"
            type="text"
            id="skills"
            name="skills"
            placeholder="Skills"
            onChange={handleSkillChange}
            value={skillsValues.skills}
          />
          <button
            onClick={handleAddSkill}
            className="px-4 text-white rounded-md mt-4 py-2 bg-clearanceGrey cursor-pointer hover:bg-clearanceDarkBlue"
          >
            Done
          </button>
        </div>
      ) : null}

      <div className="flex flex-wrap gap-2 my-6">
        {skillsData.map((data, index) => {
          return (
            <div
              key={index}
              className="flex justify-between gap-2 bg-clearanceGrey py-2 items-center rounded-lg px-4 text-white"
            >
              <h3>{data}</h3>

              <div className="flex gap-2">
                {editIndex === index ? (
                  <button
                    onClick={() => {
                      updateItem();
                    }}
                  >
                    <h2>Save</h2>
                  </button>
                ) : (
                  <button
                    onClick={() => {
                      editItem(index);
                    }}
                  >
                    <FaEdit />
                  </button>
                )}
                <button
                  onClick={() => {
                    dispatch(deleteSkill(index));
                  }}
                >
                  <FaTrash />
                </button>
                {index > 0 && (
                  <button
                    className="px-4 py-2 text-clearanceGrey bg-clearanceDarkBlue rounded"
                    onClick={() => moveItemUp(index)}
                  >
                    <FaArrowUp />
                  </button>
                )}
                {index < skillsData.length - 1 && (
                  <button
                    className="px-4 py-2 text-clearanceGrey bg-clearanceDarkBlue rounded"
                    onClick={() => moveItemDown(index)}
                  >
                    <FaArrowDown />
                  </button>
                )}
              </div>
            </div>
          );
        })}
      </div>
    </div>
  );
}

export default SkillComponent;
