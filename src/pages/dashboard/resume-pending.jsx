...
// import pdf from "https://www.pdf995.com/samples/pdf.pdf";
import { pdfjs } from "react-pdf";
import SinglePagePDFViewer from "../../common/SinglePagePDFViewer";
...
const ResumePending = () => {
  ...
  const [path, setPath] = useState("https://www.pdf995.com/samples/pdf.pdf");
  ...
  const getResume = async () => {
    await axios({
      url: "https://api.resumesquad.net/api/admin/get-resume?resume_status_id=1",
      method: "GET",
      headers: {},
    })
      .then((res) => {
  ...
  const targetUrl =
    "https://api.resumesquad.net/etc/ai_resume/user/resume_uploads/himanshu_garg_cv_improved-1712071578.pdf";
  const proxiedUrl = proxyUrl + targetUrl;

  return (
    <div>
      {showResume ? (
        <div className=" bg-black z-20 w-[100%] absolute h-[100%] bg-opacity-40">
          <div className="left-[90%] top-[5%] relative">
            <button
              onClick={() => {
                setShowResume(false);
              }}
      ...
      <DashboardLayout>
        <div className="w-full flex h-screen flex-col gap-[20px] ">
          <h2 className="text-white font-bold text-[25px]">Resumes Pending</h2>
